...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Ingestion Cost** | **$1.3812** | **$0.0081** | **$0.0006** | **$0.0000** | **[WINNER] Method 4 ($0 upfront) / Method 3 ($0.0031 with indexing)** |
| **Document Parsing & Extraction Time** | **Managed in Cloud** *(DocAI)* | **85.08s** *(Vision OCR)* | **1.24s** *(Local CPU)* | **N/A** *(Zero ETL / Native)* | **Method 3 fastest CPU parse; Method 4 requires zero ETL** |
| **Client Upload & Staging Time** | **41.69s** *(GCS + BQ)* | **N/A** *(Local)* | **N/A** *(Local)* | **N/A** *(Direct GCS URI)* | Method 1 unblocks client in ~3s; others run locally or stream |
| **Embedding Generation Time** | **41.27s** | **6.92s** | **5.60s** | **N/A** *(Zero-Embedding)* | Method 4 bypasses embeddings entirely |
| **Total Cold-Start Ingestion Time** | **84.25s** | **92.00s** | **6.84s** | **0.00s** | Total wall-clock time until ready for retrieval |
| **Query Latency (P50 / P90)** | **4748ms / 13838ms** | **3007ms / 6408ms** | **23108ms / 151298ms** | **37135ms / 199734ms** | BigQuery SQL vs In-memory vs Context window |
...
| :--- | :---: | :---: | :---:| :---: | :---: |
| **Faithfulness (Hallucination avoidance)** | 0.70 | **0.67** | **0.79** | **0.58** | **1.00** |
| **Answer Relevancy** | 0.70 | **0.84** | **0.95** | **0.82** | **0.96** |
| **Contextual Precision (Rank quality)** | 0.70 | **0.86** | **0.90** | **0.86** | **0.89** |
| **Contextual Recall (Fact coverage)** | 0.70 | **0.79** | **0.94** | **0.62** | **0.87** |
| **Contextual Relevancy (Signal-to-noise)** | 0.70 | **0.80** | **0.88** | **0.88** | **0.88** |
| **Document Completeness (Entity extraction)** | 0.70 | **0.66** | **0.79** | **0.58** | **0.82** |
...
| :--- | :---: | :---: | :---: | :---: | :---: |
| **Early Section (Primacy)** | Pages 1–15 (9 TCs) | **0.68** | **0.92** | **0.63** | **0.89** |
| **Middle Section (Valley)** | Pages 16–35 (8 TCs) | **0.87** | **0.97** | **0.64** | **0.78** |
| **Late Section (Recency)** | Pages 36–50 (8 TCs) | **0.83** | **0.93** | **0.58** | **0.94** |
| **Attentional Degradation ("Lost in Middle")** | Middle drop vs Edges | **0.0%** | **0.0%** | **0.0%** | **14.5%** (U-Curve) |
...
Method 4 requires **$0.00 upfront ingestion cost**, but incurs **~$0.001995 per query** because the entire 50-page document must be re-sent to Gemini Flash on every single interaction. In contrast, Chunked RAG (Methods 2 & 3) incurs an upfront indexing cost but only sends ~1,500 retrieved tokens per query (~$0.000079).
...
| :---: | :---: | :---: | :---: | :---: | :--- |
| **1 query** | $1.3813 | $0.0082 | $0.0007 | **$0.0020** | **Method 4 ($0.0020 total)** |
| **5 queries** | $1.3815 | $0.0085 | **$0.0010** | $0.0100 | **Method 3 Crossover (Cheaper than M4)** |
| **10 queries** | $1.3818 | $0.0089 | **$0.0014** | $0.0199 | **Method 3 is cheaper than Method 4** |
| **50 queries** | $1.3842 | $0.0121 | **$0.0046** | $0.0997 | **Method 3 & 2 dominate** |
| **200 queries** | $1.3932 | $0.0239 | **$0.0164** | $0.3990 | **Method 4 is significantly more expensive** |
| **1,000 queries** | $1.4412 | $0.0871 | **$0.0796** | $1.9950 | **Chunked RAG saves orders of magnitude** |
