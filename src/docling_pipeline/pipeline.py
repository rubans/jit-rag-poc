...
class DoclingRAGPipeline:
    """
    Method 3b: Docling Local CPU Just-In-Time RAG Pipeline.
    Leverages IBM Research Docling's TableFormer and layout model to extract high-fidelity
    structured Markdown tables on local CPU, indexing with text-embedding-004 and Gemini Flash.
    """
    def __init__(
        self,
        project_id: Optional[str] = None,
        work_dir: str = "output/docling_pipeline",
        ...
        self.metrics = ExecutionMetrics(pipeline_name="Docling Local CPU Markdown (Method 3b)")
    ...
    def ingest(self, pdf_path: str) -> ExecutionMetrics:
        ...
        # 1. Extract via Docling in isolated venv
        extract_dir = self.extract_dir
        meta_file = extract_dir / "metadata.json"
        doc_md_file = extract_dir / "document.md"

        if self.use_cache and doc_md_file.exists() and meta_file.exists():
            with open(meta_file, "r", encoding="utf-8") as f:
                meta = json.load(f)
            parse_s = meta.get("parse_time_s", 249.0)
            with open(doc_md_file, "r", encoding="utf-8") as f:
                full_md = f.read()
        else:
            extract_dir.mkdir(parents=True, exist_ok=True)
            venv_python = Path(".venv-docling/Scripts/python.exe").resolve()
            if not venv_python.exists():
                raise RuntimeError("Docling isolated environment not found at .venv-docling")
        ...
        chunks = self.chunker.chunk_text(full_md, doc_id="docling_doc")
        self.chunks = chunks
        ...
        self.metrics.total_latency_ms = parse_ms + emb_ms
    ...
    def query(self, user_query: str, top_k: int = 5) -> Tuple[str, List[RetrievalResult], ExecutionMetrics]:
        ...
        q_metrics = ExecutionMetrics(
            pipeline_name="Docling Local CPU Markdown (Method 3b)",
